...
const upsertChallenge = async ({name, description, category, flag, points}: ChallengeUpsertProps) => {
  const { data: challengeData, error: challengeError } = await supabaseServiceClient
  .from("challenges")
  .upsert([
  ...
  .upsert([
    {challenge: upsertedChallenge.id, category: category}
  ])
...
.post(async (req, res) => {
  ...
  const {data, error} = await upsertChallenge({name, description, category, flag, points})
